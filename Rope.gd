...
export(String) var tentaculo_name
...
func link_players():
	...
	var tentaculo = get_parent().get_node("Players").get_node(tentaculo_name)

	#prevents the rope from freaking out, when relocated
	yield(get_tree(), "idle_frame")
	
	player = get_parent().get_node("Players").get_node("Player")
	ppos = $Player1_Joint.global_position 
	#$Player1_Joint.node_b = player.get_path()
	# Prueba: linkear player1_join al tentaculo
	$Player1_Joint.node_b = tentaculo.get_path()
	
	player2 = get_parent().get_node("Cabeza")
